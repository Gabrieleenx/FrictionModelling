...
for i_t in tqdm(range(num_time_steps)):
    ...
    _ = sim_obj_red.get_forces(i_t*dt,y_red, fn, force_red)
...
for i_t in tqdm(range(num_time_steps)):
    ...
    _ = sim_obj_dist.get_forces(i_t*dt,y_dist, fn, force_dist)
...
for i_t in tqdm(range(num_time_steps)):
    ...
    y_obj2 = y_red2[len(sim_obj_red2.y_fric):]
    _ = sim_obj_red2.get_forces(i_t*dt,y_red2, fn, force_red2)
...
for i_t in tqdm(range(num_time_steps)):
    ...
    _ = sim_obj_dist2.get_forces(i_t*dt,y_dist2, fn, force_dist2)

    y_obj2 = y_dist2[len(sim_obj_dist2.y_fric):]
...
ax3.plot(data['t'], data['theta_red'], label="$\\theta_{r,1}$", alpha=0.7)
ax3.plot(data['t'], data['theta2'], '--', label="$\\theta_{d,2}$", alpha=0.7)
ax3.plot(data['t'], data['theta_red2'], '--',label="$\\theta_{r,2}$", alpha=0.7)
...
ax4.plot(data['t'], data['fn'], label="$f_{N}$", alpha=0.7)
