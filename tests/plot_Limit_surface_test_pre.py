...
limit_surface = CustomHashList3D(20)
limit_surface.initialize(planar_lugre)
...
for j in range(4 * num):
    ...
    for i in range(2*num):
        ...
        vel_vec = {'x': lin_vel_x_list[i], 'y': lin_vel_y_list[i], 'tau': ang_vel_list[i]}
        h = limit_surface.get_bilinear_interpolation(vel_vec, ra=0.006571420573777649)
        #f = planar_lugre.force_at_cop
        data3d[0, i + j * 2*num] = h[0,0]#f['x']
        data3d[1, i + j * 2*num] = h[1,0]#f['y']
        data3d[2, i + j * 2*num] = h[2,0]#f['tau']
